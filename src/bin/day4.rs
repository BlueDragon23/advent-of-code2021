...
fn main() {
    ...
    let moves = lines
        ...
        .collect::<Vec<_>>();
    let boards = lines.chunks(6).into_iter().map(|mut chunk| {
      // skip a blank line
      chunk.next();
      chunk.map(|line| {
        line.unwrap().split_whitespace().map(|x| x.parse::<u32>().unwrap()).collect::<Vec<_>>()
    ...
    println!("Score is {}", calculate_score(&boards[winning_board], &played_numbers, last_number));
...
fn calculate_score(board: &Vec<u32>, played_numbers: &HashSet<u32>, last_number: u32) -> u32 {
  board.into_iter().filter(|x| !played_numbers.contains(x)).sum::<u32>() * last_number
}