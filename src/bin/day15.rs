...
fn find_path(cave: &Vec<Vec<u32>>) -> u32 {
    ...
    let row_count = if PART == 1 { cave.len() } else { cave.len() * 5 };
    let col_count = if PART == 1 { cave[0].len() } else { cave[0].len() * 5 };
...
fn find_next_node(distances: &HashMap<Coordinate, u32>, visited: &HashSet<Coordinate>) -> Coordinate {
    distances
        .into_iter()
