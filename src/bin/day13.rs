...
fn main() {
    ...
    let lines = reader
        ...
        .collect_vec();
    let coordinates = lines.clone().into_iter().filter_map(|line| match line {
      Line::Coordinate(c) => Some(c),
      _ => None
    }).collect_vec();
    let instructions = lines.into_iter().filter_map(|line| match line {
      Line::Instruction(i) => Some(i),
      _ => None
    }).collect_vec();

    let final_coords = instructions.into_iter().take(if PART == 1 { 1 } else { 20 }).fold(coordinates, |coords, instruction| {
...
fn fold_paper(coordinates: &Vec<Coordinate>, instruction: Instruction) -> Vec<Coordinate> {
  let new_coordinates = match instruction {
    Instruction::Horizontal(line) => {
      let mut above = coordinates.into_iter().filter(|&c| c.y < line).map(|&c| c).collect_vec();
    ...
    Instruction::Vertical(line) => {
      let mut left = coordinates.into_iter().filter(|&c| c.x < line).map(|&c| c).collect_vec();
